%% north atlantic profile interpolation;

clear; pack

load /home/alison/Matlab/Ma't files'/Global' Mapping 2'/North' Atlantic'/prof_2005_qc.mat
fnum2 = fnum; latitude2 = latitude; longitude2 = longitude; time2 = time; type2 = type;
temp2 = NaN(574, 54172); psal2 = NaN(574, 54172); pres2 = NaN(574, 54172); datamode2 = datamode;
temp2_err = NaN(574, 54172); psal2_err = NaN(574, 54172); pres2_err = NaN(574, 54172);
[m,n] = size(pres);
...
load /home/alison/Matlab/M'at files'/Global' Mapping 2'/North' Atlantic'/prof_2006_qc.mat
...
load /home/alison/Matlab/M'at files'/Global' Mapping 2'/North' Atlantic'/prof_2007_qc.mat
fnum2 = [fnum2; fnum]; latitude2 = [latitude2; latitude]; longitude2 = [longitude2; longitude];
time2 = [time2; time]; type2 = [type2; type]; datamode2 = [datamode2; datamode];
[mmm,nnn] = size(pres);
...
load /home/alison/Matlab/M'at files'/Global' Mapping 2'/North' Atlantic'/prof_2008_qc.mat
fnum2 = [fnum2; fnum]; latitude2 = [latitude2; latitude]; longitude2 = [longitude2; longitude];
time2 = [time2; time]; type2 = [type2; type]; datamode2 = [datamode2; datamode];
[mmmm,nnnn] = size(pres);
...
load /home/alison/Matlab/M'at files'/Global' Mapping 2'/North' Atlantic'/prof_2009_qc.mat
fnum2 = [fnum2; fnum]; latitude2 = [latitude2; latitude]; longitude2 = [longitude2; longitude];
time2 = [time2; time]; type2 = [type2; type]; datamode2 = [datamode2; datamode];
[m5,n5] = size(pres);
...
load /home/alison/Matlab/M'at files'/Global' Mapping 2'/North' Atlantic'/prof_2010_qc.mat
fnum2 = [fnum2; fnum]; latitude2 = [latitude2; latitude]; longitude2 = [longitude2; longitude];
time2 = [time2; time]; type2 = [type2; type]; datamode2 = [datamode2; datamode];
[m6,n6] = size(pres);
...
save /home/alison/Matlab/Mat' files'/Global' Mapping 2'/North' Atlantic'/profiles_interp.mat
...
for i = 1:length(fnum)
    ...
    if dp(jj(end)) < 10 && dp(jj(end-8)) > 10
...
%     
%     figure(1); clf
%     plot(repmat((1:length(ind)),1015,1),-pres(:,ind),'.-');
...
clear fxnr i ibottom itop mnp mxp nn p s t rho tmppres tmp
...
%% 
clear; pack;
load /home/alison/Matlab/Mat' files'/Global' Mapping 2'/North' Atlantic'/profiles_interp.mat
fnum2 = fnum; latitude2 = latitude; longitude2 = longitude; time2 = time; type2 = type;
temp2 = temp; psal2 = psal; pres2 = pres;

load /home/alison/Matlab/Mat' files'/Global' Mapping'/North' Atlantic'/profiles_final.mat
fnum3 = fnum; latitude3 = latitude; longitude3 = longitude; time3 = time; type3 = type;
temp3 = temp; psal3 = psal; pres3 = pres;

% load /home/alison/Matlab/Mat' files'/Global' Mapping'/North' Atlantic'/profiles.mat
...
for i = 1:length(ind2)
    ...
    if max(dp) > 110
        ...
        ii = find(fnum2 == fnum2(ind2(i)));
        figure(1); clf; hold on;
        plot(psal2(:,ii),-pres2(:,ii),'k.-');
        plot(psal2(:,ind2(i)),-pres2(:,ind2(i)),'.-');
        
        figure(2); clf;
        plot(temp2(:,ii),-pres2(:,ii),'k.-'); hold on;
...
for i = 1:length(ind)
    p = pres2(:,ind(i));
    dp = p(2:end)-p(1:end-1);
    ii = find(fnum2 == fnum2(ind(i)));
    if max(dp) > 110
        figure(1); clf; hold on;
        plot(psal2(:,ii),-pres2(:,ii),'-','color',[.7 .7 .7]);
        plot(psal2(:,ind(i)),-pres2(:,ind(i)),'k.-'); grid on;
        figure(2); clf; hold on;
        plot(temp2(:,ii),-pres2(:,ii),'-','color',[.7 .7 .7]);
        plot(temp2(:,ind(i)),-pres2(:,ind(i)),'k.-'); grid on;
        disp(fnum2(ind(i)))
        figure(3); clf; hold on;
        d = sw_svan(psal2(:,ind(i)),temp2(:,ind(i)),pres2(:,ind(i)));
        dd = sw_svan(psal2(:,ii),temp2(:,ii),pres2(:,ii));
        plot(dd,-pres2(:,ii),'-','color',[.7 .7 .7]);
        plot(d,-pres2(:,ind(i)),'k.-'); grid on;
        
        disp([max(dp) p(dp==max(dp))'])
        disp(i)
        
        b = input('enter for good, 1 for bad');
        if ~isempty(b)
            bad(i) = b;
        end
    end
...
for i = 1:length(bad)
    ...
    if bad(i) == 1
        ...
        badind(i,:) = input('enter pressures to delete');
...
for i = 1:length(badind)
   if ~isnan(badind(i,1))
       ii = find(pres2(:,ind(i)) > badind(i,1) & pres2(:,ind(i)) < badind(i,2));
       pres2(ii,ind(i)) = NaN;
   end
end
...
clear b badind d dd float_num lev
...
save /home/alison/Matlab/Mat' files'/Global' Mapping 2'/North' Atlantic'/profiles_final.mat

%% misc
ind = find(fnum == 4900681);
d = sw_svan(psal(:,ind),temp(:,ind),pres(:,ind));
figure(1); clf
plot(d,-pres(:,ind),'.-');

ind = find(fnum == 6900360);
d = sw_svan(psal(:,ind),temp(:,ind),pres(:,ind));
figure(1); clf
plot(d,-pres(:,ind),'.-');

ind = find(fnum == 6900615);
d = sw_svan(psal(:,ind),temp(:,ind),pres(:,ind));
figure(1); clf
plot(d,-pres(:,ind),'.-');

ind = find(~(longitude > -71 & longitude < -70 & latitude > 41 & latitude < 42));
...
ind = find(~(longitude < -68 & latitude < 19) & ~(longitude < -62 & latitude < 17));
temp = temp(:,ind); latitude = latitude(ind); longitude = longitude(ind); time = time(ind); pres = pres(:,ind);
psal = psal(:,ind); fnum = fnum(ind); type = type(ind);
datamode = datamode(ind);
temp_err = temp_err(:,ind); pres_err = pres_err(:,ind); psal_err = psal_err(:,ind);
...
for i = 1:length(ind)
    ...
    s1 = psal(1:100,ind(i));
    s2 = psal(1:100,ind(i)+1);
    p1 = pres(1:100,ind(i));
    p2 = pres(1:100,ind(i)+1);
    figure(1); clf
    plot(t1,-p1,'.-') 
    hold on; plot(t2,-p2,'r.-');
    figure(2); clf
    plot(s1,-p1,'.-') 
    hold on; plot(s2,-p2,'r.-');
